...
public class CharacterControl : MonoBehaviour {
	...
	private void checkMovement() {
		...
		Vector3 sphereLocLeft = new Vector3(transform.position.x-0.5f,transform.position.y,transform.position.z);
		check (sphereLocLeft);
		
		Vector3 sphereLocRight = new Vector3(transform.position.x+0.5f,transform.position.y,transform.position.z);
		check (sphereLocRight);
	}
